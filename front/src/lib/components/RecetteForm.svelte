<script lang="ts">
    ...
    export let onSubmit: (recette: Recette) => Promise<void>;
...
>
    <h2 class="text-center text-2xl font-semibold text-gray-800">
        {mode === 'create' ? 'Créer une recette' : 'Modifier la recette'}
    </h2>
    ...
    <Button type="submit">{mode === 'create' ? 'Créer la recette' : 'Modifier la recette'}</Button>
</form>